...
const { data: user } = await useHttp<IUser>('user/profile');
...
const handleUploadAvatar = async () => {
  ...
  const { data, error } = await useHttp('user/upload-avatar', {
    body: formData,
    method: 'POST',
  });
...
const handleEdit = async () => {
  ...
  const { data, error } = await useHttp('user/edit', {
    body: userData,
    method: 'POST',
  });
...
const handleChangePassword = async () => {
  ...
  const { data, error } = await useHttp('user/change-password', {
    body: {
