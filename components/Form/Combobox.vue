...
<template>
  ...
  >
    <ComboboxButton
      ...
      class="min-h-[40px] relative flex w-full flex-wrap items-center gap-1 overflow-hidden rounded border-2 bg-white py-1 pr-14 text-sm focus-within:border-gray-200 focus-within:!bg-white hover:bg-gray-100 hover:border-gray-300 focus:ring-0 px-4"
    >
      ...
      <ComboboxOptions class="min-w-fit">
        ...
        <template v-if="!isLoadingOptions && !isCreatingOption">
          <ComboboxOption v-if="queryOption" v-slot="{ active }" :value="queryOption" as="template">
            <li
              :class="{
                'bg-gray-200 shadow-[inset_2px_0px_0px] shadow-emerald-600': active,
              }"
          ...
          >
            <li
              :class="{
                'shadow-[inset_2px_0px_0px] shadow-emerald-600': active || selected,
